...
1. MuJoCo 上の Go2 をテレオペで一度走らせる
   -> RGB画像、odom.csv を保存
   -> depth画像も互換用に保存できるが、通常の repeat では使わない

2. mujoco_sim/build_topomap_vision.py で topomap を作る
   -> DINO 系の視覚埋め込みで keyframe 画像列を選ぶ
...
デフォルトでは、論文の topomap generator に近い視覚埋め込みベースの keyframe 選択を使います。ローカルの DINOv3 repo を指定したい場合は次のようにします。

```bash
python3 mujoco_sim/run.py build \
  --feature-model dinov3 \
  --dinov3-repo /path/to/dinov3 \
  --dinov3-weights /path/to/dinov3_weights.pth
```

指定しない場合は、同じ構造の視覚埋め込み選択として DINOv2 fallback を使います。旧来の odometry しきい値方式で比較したい場合は `--topomap-builder odom` を付けます。
...
視覚埋め込みベースの topomap 作成には、追加で Pillow、PyTorch、torchvision、DINO 系の feature model が必要です。公式 `guidenav/navigate.py` まで動かすには、さらに OpenCV、pandas、ROS2 Humble、`cv_bridge`、GuideNav の model weights が必要です。
...
ここは GUI は出ません。`1` で保存した RGB 画像と odometry から、公式 GuideNav が読む keyframe 画像列を作ります。通常は、論文の topomap generator と同じ考え方で、DINO 系の視覚埋め込みを使って「見た目が十分変わった frame」を選びます。odometry は MuJoCo 上で開始姿勢や確認用ログを保つために残します。
...
python mujoco_sim/build_topomap_vision.py \
  "$RUN" \
...
```text
Aligned RGB/odom frames: ...
Loading DINO-style feature model: ...
Keyframe 0: ...
Vision topomap complete
```

DINOv3 repo を指定しない場合は DINOv2 fallback を使います。論文にさらに合わせたい場合は、ローカルに DINOv3 を置いて次のように指定します。
...
mujoco_sim/
  ...
  build_topomap_vision.py              DINO系視覚埋め込みでtopomapを作成
