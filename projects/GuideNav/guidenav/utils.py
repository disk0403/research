...
def read_depth_image(path):
    ...
    if depth.max() > 1000:  # likely in mm
        depth = depth / 1000.0
    return depth
...
def get_image_transform(image_size: List[int]) -> transforms.Compose:
    ...
    return transforms.Compose(
        [
            transforms.ToTensor(), # PIL/npy image to tensor (c, h, w) + normalize to [0, 1]
            transforms.Resize(image_size, antialias=True), # expects (h, w); use antialising for better quality (when downscaling)
